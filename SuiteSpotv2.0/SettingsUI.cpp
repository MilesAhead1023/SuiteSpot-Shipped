...
void SettingsUI::RenderOverlayTab() {
    ...
    if (ImGui::Button("Reset to Defaults")) {
        ...
        SetCVarSafely("overlay_width", overlayWidthValue);
        SetCVarSafely("overlay_height", overlayHeightValue);
        SetCVarSafely("overlay_alpha", overlayAlphaValue);
        SetCVarSafely("overlay_duration", postMatchDurationSecValue);
        SetCVarSafely("blue_team_hue", blueTeamHueValue);
        SetCVarSafely("orange_team_hue", orangeTeamHueValue);
        SetCVarSafely("overlay_offset_x", 0.0f);
        SetCVarSafely("overlay_offset_y", 0.0f);
        SetCVarSafely("ss_overlay_mode", 0);
    }
    if (ImGui::IsItemHovered()) {
    ...
    if (ImGui::Button("Test Overlay")) {
        plugin_->ToggleTestOverlay();
    }
    if (ImGui::IsItemHovered()) {
        ImGui::SetTooltip("Show/Hide the overlay with mock data for previewing changes");
    }
    ...
    if (auto* tm = plugin_->GetThemeManager()) {
        std::vector<std::string> themes = tm->GetThemeNames();
        std::string currentTheme = tm->GetActiveTheme().name;

        if (ImGui::BeginCombo("Select Theme", currentTheme.c_str())) {
            for (const auto& themeName : themes) {
                bool isSelected = (currentTheme == themeName);
                if (ImGui::Selectable(themeName.c_str(), isSelected)) {
                    tm->ChangeTheme(themeName);
                }
                if (isSelected) {
    ...
    if (ImGui::CollapsingHeader("Window Layout", ImGuiTreeNodeFlags_DefaultOpen)) {
        // View Mode Selector
        int viewMode = plugin_->GetOverlayMode();
        ImGui::Text("View Mode:");
        ImGui::SameLine();
        if (ImGui::RadioButton("Last Match", viewMode == 0)) {
            SetCVarSafely("ss_overlay_mode", 0);
        }
        ImGui::SameLine();
        if (ImGui::RadioButton("Session Stats", viewMode == 1)) {
            SetCVarSafely("ss_overlay_mode", 1);
        }
        ImGui::Spacing();
        ...
        if (ImGui::SliderFloat("Offset X##overlay", &offsetX, -1000.0f, 1000.0f, "%.0f")) {
            SetCVarSafely("overlay_offset_x", offsetX);
        }
        ImGui::SameLine();
        ImGui::SetNextItemWidth(180);
        if (ImGui::SliderFloat("Offset Y##overlay", &offsetY, -500.0f, 500.0f, "%.0f")) {
            SetCVarSafely("overlay_offset_y", offsetY);
        }
    }
