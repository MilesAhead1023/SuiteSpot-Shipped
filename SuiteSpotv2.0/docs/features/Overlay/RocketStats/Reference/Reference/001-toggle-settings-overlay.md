# Implementation Spec: Toggle settings overlay

**Capability ID**: 1 | **Slug**: toggle-settings-overlay

## Overview

This capability allows users to configure toggle settings overlay via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **Notifier**: `rs_toggle_menu` - Game notifier callback
- **CVar Name**: `cl_rocketstats_settings`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:413-415 (registerNotifier rs_toggle_menu), RocketStats.cpp:473 (CVar cl_rocketstats_settings)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_toggle_settings_overlay", "default_value",
...
```json
{
    "rs_toggle_settings_overlay": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_toggle_settings_overlay").getValue<Type>();
```
