# Implementation Spec: Select aggregation mode for statistics

**Capability ID**: 4 | **Slug**: select-stats-aggregation-mode

## Overview

This capability allows users to configure select aggregation mode for statistics via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_select_stats_aggregation_mode`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:481 (CVar rs_mode with options: session, per-playlist, all-time, all-time per-gamemode)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_select_stats_aggregation_mode", "default_value",
...
```json
{
    "rs_select_stats_aggregation_mode": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_select_stats_aggregation_mode").getValue<Type>();
```
