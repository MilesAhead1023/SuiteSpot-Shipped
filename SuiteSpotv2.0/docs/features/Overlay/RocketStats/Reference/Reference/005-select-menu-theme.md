# Implementation Spec: Select theme for menu display

**Capability ID**: 5 | **Slug**: select-menu-theme

## Overview

This capability allows users to configure select theme for menu display via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_select_menu_theme`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:482-485 (CVar rs_theme with addOnValueChanged to ChangeTheme)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_select_menu_theme", "default_value",
...
```json
{
    "rs_select_menu_theme": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_select_menu_theme").getValue<Type>();
```
