# Implementation Spec: Select theme for in-game display

**Capability ID**: 6 | **Slug**: select-ingame-theme

## Overview

This capability allows users to configure select theme for in-game display via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_select_ingame_theme`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:487-490 (CVar rs_gameTheme, dual-theme support)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_select_ingame_theme", "default_value",
...
```json
{
    "rs_select_ingame_theme": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_select_ingame_theme").getValue<Type>();
```
