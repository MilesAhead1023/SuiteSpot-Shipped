# Implementation Spec: Set overlay X position

**Capability ID**: 7 | **Slug**: set-overlay-x-position

## Overview

This capability allows users to configure set overlay x position via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_set_overlay_x_position`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:492 (CVar rs_x, range 0.0-1.0)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_set_overlay_x_position", "default_value",
...
```json
{
    "rs_set_overlay_x_position": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_set_overlay_x_position").getValue<Type>();
```
