# Implementation Spec: Set overlay Y position

**Capability ID**: 8 | **Slug**: set-overlay-y-position

## Overview

This capability allows users to configure set overlay y position via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_set_overlay_y_position`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:493 (CVar rs_y, range 0.0-1.0)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_set_overlay_y_position", "default_value",
...
```json
{
    "rs_set_overlay_y_position": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_set_overlay_y_position").getValue<Type>();
```
