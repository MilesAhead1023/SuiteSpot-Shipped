# Implementation Spec: Set overlay scale

**Capability ID**: 9 | **Slug**: set-overlay-scale

## Overview

This capability allows users to configure set overlay scale via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_set_overlay_scale`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:494 (CVar rs_scale, range 0.001-10.0)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_set_overlay_scale", "default_value",
...
```json
{
    "rs_set_overlay_scale": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_set_overlay_scale").getValue<Type>();
```
