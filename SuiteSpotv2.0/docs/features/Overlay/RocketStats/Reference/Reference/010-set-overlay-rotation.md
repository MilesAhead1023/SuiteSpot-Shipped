# Implementation Spec: Set overlay rotation angle

**Capability ID**: 10 | **Slug**: set-overlay-rotation

## Overview

This capability allows users to configure set overlay rotation angle via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_set_overlay_rotation`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:495 (CVar rs_rotate, range -180 to 180 degrees)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_set_overlay_rotation", "default_value",
...
```json
{
    "rs_set_overlay_rotation": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_set_overlay_rotation").getValue<Type>();
```
