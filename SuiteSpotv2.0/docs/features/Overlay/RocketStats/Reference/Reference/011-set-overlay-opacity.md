# Implementation Spec: Set overlay opacity

**Capability ID**: 11 | **Slug**: set-overlay-opacity

## Overview

This capability allows users to configure set overlay opacity via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_set_overlay_opacity`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:496 (CVar rs_opacity, range 0.0-1.0)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_set_overlay_opacity", "default_value",
...
```json
{
    "rs_set_overlay_opacity": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_set_overlay_opacity").getValue<Type>();
```
