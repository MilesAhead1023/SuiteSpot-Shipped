# Implementation Spec: Show overlay in main menu

**Capability ID**: 13 | **Slug**: show-overlay-in-menu

## Overview

This capability allows users to configure show overlay in main menu via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_show_overlay_in_menu`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:500 (CVar rs_enable_inmenu)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_show_overlay_in_menu", "default_value",
...
```json
{
    "rs_show_overlay_in_menu": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_show_overlay_in_menu").getValue<Type>();
```
