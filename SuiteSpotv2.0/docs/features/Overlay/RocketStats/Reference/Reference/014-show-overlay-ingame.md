# Implementation Spec: Show overlay in-game

**Capability ID**: 14 | **Slug**: show-overlay-ingame

## Overview

This capability allows users to configure show overlay in-game via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_show_overlay_ingame`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:501 (CVar rs_enable_ingame)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_show_overlay_ingame", "default_value",
...
```json
{
    "rs_show_overlay_ingame": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_show_overlay_ingame").getValue<Type>();
```
