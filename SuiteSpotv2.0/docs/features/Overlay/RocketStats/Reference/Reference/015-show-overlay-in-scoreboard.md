# Implementation Spec: Show overlay in scoreboard

**Capability ID**: 15 | **Slug**: show-overlay-in-scoreboard

## Overview

This capability allows users to configure show overlay in scoreboard via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_show_overlay_in_scoreboard`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:502 (CVar rs_enable_inscoreboard)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_show_overlay_in_scoreboard", "default_value",
...
```json
{
    "rs_show_overlay_in_scoreboard": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_show_overlay_in_scoreboard").getValue<Type>();
```
