# Implementation Spec: Enable floating point MMR precision

**Capability ID**: 16 | **Slug**: enable-floating-point-mmr

## Overview

This capability allows users to configure enable floating point mmr precision via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_enable_floating_point_mmr`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:503 (CVar rs_enable_float, for OBS streaming)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_enable_floating_point_mmr", "default_value",
...
```json
{
    "rs_enable_floating_point_mmr": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_enable_floating_point_mmr").getValue<Type>();
```
