# Implementation Spec: Preview next rank tier

**Capability ID**: 17 | **Slug**: preview-next-rank

## Overview

This capability allows users to configure preview next rank tier via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_preview_next_rank`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:504 (CVar rs_preview_rank)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_preview_next_rank", "default_value",
...
```json
{
    "rs_preview_next_rank": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_preview_next_rank").getValue<Type>();
```
