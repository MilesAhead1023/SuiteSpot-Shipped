# Implementation Spec: Display rank division as Roman numerals

**Capability ID**: 18 | **Slug**: use-roman-numerals-division

## Overview

This capability allows users to configure display rank division as roman numerals via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_use_roman_numerals_division`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:505 (CVar rs_roman_numbers)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_use_roman_numerals_division", "default_value",
...
```json
{
    "rs_use_roman_numerals_division": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_use_roman_numerals_division").getValue<Type>();
```
