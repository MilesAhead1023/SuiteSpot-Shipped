# Implementation Spec: Replace MMR display with MMRChange

**Capability ID**: 19 | **Slug**: replace-mmr-with-mmrchange

## Overview

This capability allows users to configure replace mmr display with mmrchange via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_replace_mmr_with_mmrchange`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:507 (CVar rs_replace_mmr)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_replace_mmr_with_mmrchange", "default_value",
...
```json
{
    "rs_replace_mmr_with_mmrchange": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_replace_mmr_with_mmrchange").getValue<Type>();
```
