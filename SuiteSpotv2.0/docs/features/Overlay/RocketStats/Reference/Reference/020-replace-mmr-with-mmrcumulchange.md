# Implementation Spec: Replace MMR with MMRCumulChange

**Capability ID**: 20 | **Slug**: replace-mmr-with-mmrcumulchange

## Overview

This capability allows users to configure replace mmr with mmrcumulchange via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_replace_mmr_with_mmrcumulchange`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:508 (CVar rs_replace_mmr_cc)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_replace_mmr_with_mmrcumulchange", "default_value",
...
```json
{
    "rs_replace_mmr_with_mmrcumulchange": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_replace_mmr_with_mmrcumulchange").getValue<Type>();
```
