# Implementation Spec: Replace MMRChange with MMR

**Capability ID**: 21 | **Slug**: replace-mmrchange-with-mmr

## Overview

This capability allows users to configure replace mmrchange with mmr via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_replace_mmrchange_with_mmr`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:509 (CVar rs_replace_mmrc)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_replace_mmrchange_with_mmr", "default_value",
...
```json
{
    "rs_replace_mmrchange_with_mmr": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_replace_mmrchange_with_mmr").getValue<Type>();
```
