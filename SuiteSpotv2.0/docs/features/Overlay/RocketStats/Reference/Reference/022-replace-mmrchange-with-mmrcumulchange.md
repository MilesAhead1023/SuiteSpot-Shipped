# Implementation Spec: Replace MMRChange with MMRCumulChange

**Capability ID**: 22 | **Slug**: replace-mmrchange-with-mmrcumulchange

## Overview

This capability allows users to configure replace mmrchange with mmrcumulchange via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_replace_mmrchange_with_mmrcumulchange`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:510 (CVar rs_replace_mmrc_cc)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_replace_mmrchange_with_mmrcumulchange", "default_value",
...
```json
{
    "rs_replace_mmrchange_with_mmrcumulchange": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_replace_mmrchange_with_mmrcumulchange").getValue<Type>();
```
