# Implementation Spec: Enable file output mode

**Capability ID**: 25 | **Slug**: enable-file-output

## Overview

This capability allows users to configure enable file output mode via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_enable_file_output`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:514 (CVar rs_in_file, master toggle for all text file outputs)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_enable_file_output", "default_value",
...
```json
{
    "rs_enable_file_output": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_enable_file_output").getValue<Type>();
```
