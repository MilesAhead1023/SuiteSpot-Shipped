# Implementation Spec: Toggle file output for games count

**Capability ID**: 26 | **Slug**: toggle-file-output-games

## Overview

This capability allows users to configure toggle file output for games count via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_toggle_file_output_games`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:519 (CVar rs_file_games)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_toggle_file_output_games", "default_value",
...
```json
{
    "rs_toggle_file_output_games": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_toggle_file_output_games").getValue<Type>();
```
