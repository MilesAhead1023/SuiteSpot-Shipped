# Implementation Spec: Toggle file output for game mode

**Capability ID**: 27 | **Slug**: toggle-file-output-gamemode

## Overview

This capability allows users to configure toggle file output for game mode via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_toggle_file_output_gamemode`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:520 (CVar rs_file_gm)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_toggle_file_output_gamemode", "default_value",
...
```json
{
    "rs_toggle_file_output_gamemode": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_toggle_file_output_gamemode").getValue<Type>();
```
