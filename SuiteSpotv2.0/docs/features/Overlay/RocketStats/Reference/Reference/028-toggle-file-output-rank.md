# Implementation Spec: Toggle file output for rank

**Capability ID**: 28 | **Slug**: toggle-file-output-rank

## Overview

This capability allows users to configure toggle file output for rank via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_toggle_file_output_rank`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:521 (CVar rs_file_rank)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_toggle_file_output_rank", "default_value",
...
```json
{
    "rs_toggle_file_output_rank": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_toggle_file_output_rank").getValue<Type>();
```
