# Implementation Spec: Toggle file output for division

**Capability ID**: 29 | **Slug**: toggle-file-output-division

## Overview

This capability allows users to configure toggle file output for division via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_toggle_file_output_division`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:522 (CVar rs_file_div)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_toggle_file_output_division", "default_value",
...
```json
{
    "rs_toggle_file_output_division": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_toggle_file_output_division").getValue<Type>();
```
