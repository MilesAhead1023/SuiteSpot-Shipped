# Implementation Spec: Toggle file output for MMR

**Capability ID**: 30 | **Slug**: toggle-file-output-mmr

## Overview

This capability allows users to configure toggle file output for mmr via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_toggle_file_output_mmr`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:523 (CVar rs_file_mmr)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_toggle_file_output_mmr", "default_value",
...
```json
{
    "rs_toggle_file_output_mmr": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_toggle_file_output_mmr").getValue<Type>();
```
