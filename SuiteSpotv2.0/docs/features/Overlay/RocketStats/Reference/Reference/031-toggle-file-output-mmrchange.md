# Implementation Spec: Toggle file output for MMR change

**Capability ID**: 31 | **Slug**: toggle-file-output-mmrchange

## Overview

This capability allows users to configure toggle file output for mmr change via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_toggle_file_output_mmrchange`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:524 (CVar rs_file_mmrc)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_toggle_file_output_mmrchange", "default_value",
...
```json
{
    "rs_toggle_file_output_mmrchange": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_toggle_file_output_mmrchange").getValue<Type>();
```
