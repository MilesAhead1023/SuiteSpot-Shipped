# Implementation Spec: Toggle file output for cumulative MMR change

**Capability ID**: 32 | **Slug**: toggle-file-output-mmrcumulchange

## Overview

This capability allows users to configure toggle file output for cumulative mmr change via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_toggle_file_output_mmrcumulchange`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:525 (CVar rs_file_mmrcc)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_toggle_file_output_mmrcumulchange", "default_value",
...
```json
{
    "rs_toggle_file_output_mmrcumulchange": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_toggle_file_output_mmrcumulchange").getValue<Type>();
```
