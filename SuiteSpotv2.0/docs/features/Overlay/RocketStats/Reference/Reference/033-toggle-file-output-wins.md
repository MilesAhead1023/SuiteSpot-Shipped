# Implementation Spec: Toggle file output for wins

**Capability ID**: 33 | **Slug**: toggle-file-output-wins

## Overview

This capability allows users to configure toggle file output for wins via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_toggle_file_output_wins`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:526 (CVar rs_file_win)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_toggle_file_output_wins", "default_value",
...
```json
{
    "rs_toggle_file_output_wins": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_toggle_file_output_wins").getValue<Type>();
```
