# Implementation Spec: Toggle file output for losses

**Capability ID**: 34 | **Slug**: toggle-file-output-losses

## Overview

This capability allows users to configure toggle file output for losses via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_toggle_file_output_losses`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:527 (CVar rs_file_loss)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_toggle_file_output_losses", "default_value",
...
```json
{
    "rs_toggle_file_output_losses": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_toggle_file_output_losses").getValue<Type>();
```
