# Implementation Spec: Toggle file output for winning streak

**Capability ID**: 35 | **Slug**: toggle-file-output-streak

## Overview

This capability allows users to configure toggle file output for winning streak via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_toggle_file_output_streak`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:528 (CVar rs_file_streak)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_toggle_file_output_streak", "default_value",
...
```json
{
    "rs_toggle_file_output_streak": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_toggle_file_output_streak").getValue<Type>();
```
