# Implementation Spec: Toggle file output for win ratio

**Capability ID**: 36 | **Slug**: toggle-file-output-win-ratio

## Overview

This capability allows users to configure toggle file output for win ratio via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_toggle_file_output_win_ratio`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:529 (CVar rs_file_winratio)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_toggle_file_output_win_ratio", "default_value",
...
```json
{
    "rs_toggle_file_output_win_ratio": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_toggle_file_output_win_ratio").getValue<Type>();
```
