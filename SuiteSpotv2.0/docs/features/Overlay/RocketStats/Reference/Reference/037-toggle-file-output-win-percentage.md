# Implementation Spec: Toggle file output for win percentage

**Capability ID**: 37 | **Slug**: toggle-file-output-win-percentage

## Overview

This capability allows users to configure toggle file output for win percentage via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_toggle_file_output_win_percentage`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:530 (CVar rs_file_winpercentage)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_toggle_file_output_win_percentage", "default_value",
...
```json
{
    "rs_toggle_file_output_win_percentage": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_toggle_file_output_win_percentage").getValue<Type>();
```
