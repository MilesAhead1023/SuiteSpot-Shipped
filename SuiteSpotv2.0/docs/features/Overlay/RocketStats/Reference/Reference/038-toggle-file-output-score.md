# Implementation Spec: Toggle file output for match score

**Capability ID**: 38 | **Slug**: toggle-file-output-score

## Overview

This capability allows users to configure toggle file output for match score via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_toggle_file_output_score`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:531 (CVar rs_file_score)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_toggle_file_output_score", "default_value",
...
```json
{
    "rs_toggle_file_output_score": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_toggle_file_output_score").getValue<Type>();
```
