# Implementation Spec: Toggle file output for shot statistics

**Capability ID**: 39 | **Slug**: toggle-file-output-shots

## Overview

This capability allows users to configure toggle file output for shot statistics via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_toggle_file_output_shots`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:532 (CVar rs_file_shots)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_toggle_file_output_shots", "default_value",
...
```json
{
    "rs_toggle_file_output_shots": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_toggle_file_output_shots").getValue<Type>();
```
