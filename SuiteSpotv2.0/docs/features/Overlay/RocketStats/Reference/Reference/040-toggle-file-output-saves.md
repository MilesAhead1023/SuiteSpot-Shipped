# Implementation Spec: Toggle file output for save statistics

**Capability ID**: 40 | **Slug**: toggle-file-output-saves

## Overview

This capability allows users to configure toggle file output for save statistics via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_toggle_file_output_saves`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:533 (CVar rs_file_saves)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_toggle_file_output_saves", "default_value",
...
```json
{
    "rs_toggle_file_output_saves": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_toggle_file_output_saves").getValue<Type>();
```
