# Implementation Spec: Toggle file output for goal statistics

**Capability ID**: 41 | **Slug**: toggle-file-output-goals

## Overview

This capability allows users to configure toggle file output for goal statistics via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_toggle_file_output_goals`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:534 (CVar rs_file_goals)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_toggle_file_output_goals", "default_value",
...
```json
{
    "rs_toggle_file_output_goals": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_toggle_file_output_goals").getValue<Type>();
```
