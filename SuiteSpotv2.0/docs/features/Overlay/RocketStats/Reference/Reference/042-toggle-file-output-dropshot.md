# Implementation Spec: Toggle file output for dropshot statistics

**Capability ID**: 42 | **Slug**: toggle-file-output-dropshot

## Overview

This capability allows users to configure toggle file output for dropshot statistics via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_toggle_file_output_dropshot`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:535 (CVar rs_file_dropshot)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_toggle_file_output_dropshot", "default_value",
...
```json
{
    "rs_toggle_file_output_dropshot": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_toggle_file_output_dropshot").getValue<Type>();
```
