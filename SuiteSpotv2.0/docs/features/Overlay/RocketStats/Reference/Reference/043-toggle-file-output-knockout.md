# Implementation Spec: Toggle file output for knockout statistics

**Capability ID**: 43 | **Slug**: toggle-file-output-knockout

## Overview

This capability allows users to configure toggle file output for knockout statistics via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_toggle_file_output_knockout`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:536 (CVar rs_file_knockout)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_toggle_file_output_knockout", "default_value",
...
```json
{
    "rs_toggle_file_output_knockout": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_toggle_file_output_knockout").getValue<Type>();
```
