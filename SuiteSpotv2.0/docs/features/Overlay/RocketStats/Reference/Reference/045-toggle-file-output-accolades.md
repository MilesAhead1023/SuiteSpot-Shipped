# Implementation Spec: Toggle file output for accolades

**Capability ID**: 45 | **Slug**: toggle-file-output-accolades

## Overview

This capability allows users to configure toggle file output for accolades via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_toggle_file_output_accolades`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:538 (CVar rs_file_accolades)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_toggle_file_output_accolades", "default_value",
...
```json
{
    "rs_toggle_file_output_accolades": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_toggle_file_output_accolades").getValue<Type>();
```
