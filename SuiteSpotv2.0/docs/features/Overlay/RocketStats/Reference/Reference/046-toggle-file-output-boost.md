# Implementation Spec: Toggle file output for boost state

**Capability ID**: 46 | **Slug**: toggle-file-output-boost

## Overview

This capability allows users to configure toggle file output for boost state via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_toggle_file_output_boost`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:539 (CVar rs_file_boost)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_toggle_file_output_boost", "default_value",
...
```json
{
    "rs_toggle_file_output_boost": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_toggle_file_output_boost").getValue<Type>();
```
