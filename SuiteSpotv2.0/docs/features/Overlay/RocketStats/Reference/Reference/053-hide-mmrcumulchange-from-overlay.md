# Implementation Spec: Hide cumulative MMR change from overlay

**Capability ID**: 53 | **Slug**: hide-mmrcumulchange-from-overlay

## Overview

This capability allows users to configure hide cumulative mmr change from overlay via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_hide_mmrcumulchange_from_overlay`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:547 (CVar rs_hide_mmrcc)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_hide_mmrcumulchange_from_overlay", "default_value",
...
```json
{
    "rs_hide_mmrcumulchange_from_overlay": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_hide_mmrcumulchange_from_overlay").getValue<Type>();
```
