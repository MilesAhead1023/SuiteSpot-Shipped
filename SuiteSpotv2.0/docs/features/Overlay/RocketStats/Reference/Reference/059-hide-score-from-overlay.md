# Implementation Spec: Hide match score from overlay

**Capability ID**: 59 | **Slug**: hide-score-from-overlay

## Overview

This capability allows users to configure hide match score from overlay via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_hide_score_from_overlay`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:553 (CVar rs_hide_score)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_hide_score_from_overlay", "default_value",
...
```json
{
    "rs_hide_score_from_overlay": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_hide_score_from_overlay").getValue<Type>();
```
