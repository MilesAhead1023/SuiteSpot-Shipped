# Implementation Spec: Hide knockout statistics from overlay

**Capability ID**: 64 | **Slug**: hide-knockout-from-overlay

## Overview

This capability allows users to configure hide knockout statistics from overlay via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_hide_knockout_from_overlay`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:558 (CVar rs_hide_knockout)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_hide_knockout_from_overlay", "default_value",
...
```json
{
    "rs_hide_knockout_from_overlay": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_hide_knockout_from_overlay").getValue<Type>();
```
