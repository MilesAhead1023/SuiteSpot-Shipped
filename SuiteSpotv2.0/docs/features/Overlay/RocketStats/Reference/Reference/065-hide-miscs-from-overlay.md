# Implementation Spec: Hide miscellaneous statistics from overlay

**Capability ID**: 65 | **Slug**: hide-miscs-from-overlay

## Overview

This capability allows users to configure hide miscellaneous statistics from overlay via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_hide_miscs_from_overlay`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:559 (CVar rs_hide_miscs)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_hide_miscs_from_overlay", "default_value",
...
```json
{
    "rs_hide_miscs_from_overlay": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_hide_miscs_from_overlay").getValue<Type>();
```
