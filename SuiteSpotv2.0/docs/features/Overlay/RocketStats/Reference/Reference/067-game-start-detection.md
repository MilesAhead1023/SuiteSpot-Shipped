# Implementation Spec: Detect and track match start

**Capability ID**: 67 | **Slug**: game-start-detection

## Overview

This capability hooks into Rocket League game events to detect detect and track match start. The hook is registered during plugin initialization and fires whenever the corresponding event occurs.
...
- **Evidence**: RocketStats.cpp:434 (HookEvent Countdown.BeginState), GameManagement.cpp:3 (GameStart)
