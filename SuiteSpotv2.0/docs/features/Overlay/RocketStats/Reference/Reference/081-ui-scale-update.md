# Implementation Spec: Update UI scale on resolution change

**Capability ID**: 81 | **Slug**: ui-scale-update

## Overview

This capability hooks into Rocket League game events to detect update ui scale on resolution change. The hook is registered during plugin initialization and fires whenever the corresponding event occurs.
...
- **Evidence**: RocketStats.cpp:432-433 (HookEvent SetUIScaleModifier, IsFullScreenViewport)
