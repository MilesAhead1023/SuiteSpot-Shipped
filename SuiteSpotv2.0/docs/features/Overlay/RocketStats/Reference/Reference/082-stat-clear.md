# Implementation Spec: Increment clear counter

**Capability ID**: 82 | **Slug**: stat-clear

## Overview

This capability tracks increment clear counter in Rocket League matches. The statistic is accumulated across four aggregation levels (session, per-playlist, all-time, all-time per-gamemode) and synchronized with file output and WebSocket streams.
...
- **Evidence**: StatsManagement.cpp (onStatEvent with Clear stat type), Vars/Shots.cpp::VarShotsClear
