# Implementation Spec: Increment assist counter

**Capability ID**: 83 | **Slug**: stat-assist

## Overview

This capability tracks increment assist counter in Rocket League matches. The statistic is accumulated across four aggregation levels (session, per-playlist, all-time, all-time per-gamemode) and synchronized with file output and WebSocket streams.
...
- **Evidence**: StatsManagement.cpp (onStatEvent with Assist stat type), Vars/Shots.cpp::VarShotsAssist
