# Implementation Spec: Increment bicycle hit counter

**Capability ID**: 86 | **Slug**: stat-bicycle-hit

## Overview

This capability tracks increment bicycle hit counter in Rocket League matches. The statistic is accumulated across four aggregation levels (session, per-playlist, all-time, all-time per-gamemode) and synchronized with file output and WebSocket streams.
...
- **Evidence**: StatsManagement.cpp (onStatEvent), Vars/Shots.cpp::VarShotsBicycleHit
