# Implementation Spec: Track save counter

**Capability ID**: 89 | **Slug**: stat-save

## Overview

This capability tracks track save counter in Rocket League matches. The statistic is accumulated across four aggregation levels (session, per-playlist, all-time, all-time per-gamemode) and synchronized with file output and WebSocket streams.
...
- **Evidence**: StatsManagement.cpp (onStatEvent with Save type), Vars/Saves.cpp::VarSavesSave
