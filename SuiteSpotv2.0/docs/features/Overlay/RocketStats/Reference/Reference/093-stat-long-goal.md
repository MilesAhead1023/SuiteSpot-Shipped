# Implementation Spec: Track long goal counter

**Capability ID**: 93 | **Slug**: stat-long-goal

## Overview

This capability tracks track long goal counter in Rocket League matches. The statistic is accumulated across four aggregation levels (session, per-playlist, all-time, all-time per-gamemode) and synchronized with file output and WebSocket streams.
...
- **Evidence**: StatsManagement.cpp (onStatEvent), Vars/Goals.cpp::VarGoalsLongGoal
