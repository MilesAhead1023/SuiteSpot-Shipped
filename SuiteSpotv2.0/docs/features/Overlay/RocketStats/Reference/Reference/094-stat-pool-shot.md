# Implementation Spec: Track pool shot counter

**Capability ID**: 94 | **Slug**: stat-pool-shot

## Overview

This capability tracks track pool shot counter in Rocket League matches. The statistic is accumulated across four aggregation levels (session, per-playlist, all-time, all-time per-gamemode) and synchronized with file output and WebSocket streams.
...
- **Evidence**: StatsManagement.cpp (onStatEvent), Vars/Goals.cpp::VarGoalsPoolShot
