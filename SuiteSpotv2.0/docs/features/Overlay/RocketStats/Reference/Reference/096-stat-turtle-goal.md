# Implementation Spec: Track turtle goal counter

**Capability ID**: 96 | **Slug**: stat-turtle-goal

## Overview

This capability tracks track turtle goal counter in Rocket League matches. The statistic is accumulated across four aggregation levels (session, per-playlist, all-time, all-time per-gamemode) and synchronized with file output and WebSocket streams.
...
- **Evidence**: StatsManagement.cpp (onStatEvent), Vars/Goals.cpp::VarGoalsTurtleGoal
