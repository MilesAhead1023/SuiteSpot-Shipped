# Implementation Spec: Track bicycle goal counter

**Capability ID**: 97 | **Slug**: stat-bicycle-goal

## Overview

This capability tracks track bicycle goal counter in Rocket League matches. The statistic is accumulated across four aggregation levels (session, per-playlist, all-time, all-time per-gamemode) and synchronized with file output and WebSocket streams.
...
- **Evidence**: StatsManagement.cpp (onStatEvent), Vars/Goals.cpp::VarGoalsBicycleGoal
