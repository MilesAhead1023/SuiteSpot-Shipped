# Implementation Spec: Track backwards goal counter

**Capability ID**: 99 | **Slug**: stat-backwards-goal

## Overview

This capability tracks track backwards goal counter in Rocket League matches. The statistic is accumulated across four aggregation levels (session, per-playlist, all-time, all-time per-gamemode) and synchronized with file output and WebSocket streams.
...
- **Evidence**: StatsManagement.cpp (onStatEvent), Vars/Goals.cpp::VarGoalsBackwardsGoal
