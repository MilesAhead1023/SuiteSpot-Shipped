# Implementation Spec: Track hoops swish goal counter

**Capability ID**: 100 | **Slug**: stat-hoops-swish-goal

## Overview

This capability tracks track hoops swish goal counter in Rocket League matches. The statistic is accumulated across four aggregation levels (session, per-playlist, all-time, all-time per-gamemode) and synchronized with file output and WebSocket streams.
...
- **Evidence**: StatsManagement.cpp (onStatEvent), Vars/Goals.cpp::VarGoalsHoopsSwishGoal
