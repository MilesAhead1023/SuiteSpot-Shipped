# Implementation Spec: Track large breakout damage counter

**Capability ID**: 102 | **Slug**: stat-breakout-damage-large

## Overview

This capability tracks track large breakout damage counter in Rocket League matches. The statistic is accumulated across four aggregation levels (session, per-playlist, all-time, all-time per-gamemode) and synchronized with file output and WebSocket streams.
...
- **Evidence**: StatsManagement.cpp (onStatEvent), Vars/Dropshot.cpp::VarDropshotBreakoutDamageLarge
