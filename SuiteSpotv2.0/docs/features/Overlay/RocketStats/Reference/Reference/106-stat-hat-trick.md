# Implementation Spec: Track hat trick achievement

**Capability ID**: 106 | **Slug**: stat-hat-trick

## Overview

This capability tracks track hat trick achievement in Rocket League matches. The statistic is accumulated across four aggregation levels (session, per-playlist, all-time, all-time per-gamemode) and synchronized with file output and WebSocket streams.
...
- **Evidence**: StatsManagement.cpp (onStatTickerMessage), Vars/Miscs.cpp::VarMiscsHatTrick
