# Implementation Spec: Track win counter

**Capability ID**: 110 | **Slug**: stat-win

## Overview

This capability tracks track win counter in Rocket League matches. The statistic is accumulated across four aggregation levels (session, per-playlist, all-time, all-time per-gamemode) and synchronized with file output and WebSocket streams.
...
- **Evidence**: GameManagement.cpp::GameEnd, RocketStats.h (Stats.win)
