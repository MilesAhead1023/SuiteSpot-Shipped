# Implementation Spec: Track loss counter

**Capability ID**: 111 | **Slug**: stat-loss

## Overview

This capability tracks track loss counter in Rocket League matches. The statistic is accumulated across four aggregation levels (session, per-playlist, all-time, all-time per-gamemode) and synchronized with file output and WebSocket streams.
...
- **Evidence**: GameManagement.cpp::GameEnd, RocketStats.h (Stats.loss)
