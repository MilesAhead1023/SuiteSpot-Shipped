# Implementation Spec: Track current game rank and division

**Capability ID**: 114 | **Slug**: track-current-rank-division

## Overview

This capability tracks track current game rank and division in Rocket League matches. The statistic is accumulated across four aggregation levels (session, per-playlist, all-time, all-time per-gamemode) and synchronized with file output and WebSocket streams.
...
- **Evidence**: StatsManagement.cpp::MajRank, RocketStats.h (current struct with rank/division)
