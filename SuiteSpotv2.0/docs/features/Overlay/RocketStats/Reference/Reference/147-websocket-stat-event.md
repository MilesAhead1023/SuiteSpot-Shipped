# Implementation Spec: Send stat event updates via WebSocket

**Capability ID**: 147 | **Slug**: websocket-stat-event

## Overview

This capability manages WebSocket communication for send stat event updates via websocket. Messages are broadcast to OBS clients on port 8085 in JSON format.
...
- **Evidence**: StatsManagement.cpp::onStatEvent (SocketSend updates)
