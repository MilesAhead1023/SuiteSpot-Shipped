# Implementation Spec: Refresh overlay on setting change

**Capability ID**: 164 | **Slug**: refresh-overlay-on-change

## Overview

This capability allows users to configure refresh overlay on setting change via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_refresh_overlay_on_change`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp (RefreshTheme callback on multiple CVars)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_refresh_overlay_on_change", "default_value",
...
```json
{
    "rs_refresh_overlay_on_change": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_refresh_overlay_on_change").getValue<Type>();
```
