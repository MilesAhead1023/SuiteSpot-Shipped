# Implementation Spec: Update file outputs on setting change

**Capability ID**: 165 | **Slug**: update-files-on-change

## Overview

This capability allows users to configure update file outputs on setting change via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_update_files_on_change`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp (RefreshFiles callback)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_update_files_on_change", "default_value",
...
```json
{
    "rs_update_files_on_change": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_update_files_on_change").getValue<Type>();
```
