# Implementation Spec: Display boost amount percentage

**Capability ID**: 179 | **Slug**: display-boost-amount

## Overview

This capability tracks display boost amount percentage in Rocket League matches. The statistic is accumulated across four aggregation levels (session, per-playlist, all-time, all-time per-gamemode) and synchronized with file output and WebSocket streams.
...
- **Evidence**: Vars/Other.cpp::VarBoost
