# Implementation Spec: Support dual-theme menu vs in-game display

**Capability ID**: 191 | **Slug**: dual-theme-support

## Overview

This capability allows users to configure support dual-theme menu vs in-game display via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_dual_theme_support`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.h (dualtheme flag, rs_theme vs rs_gameTheme)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_dual_theme_support", "default_value",
...
```json
{
    "rs_dual_theme_support": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_dual_theme_support").getValue<Type>();
```
