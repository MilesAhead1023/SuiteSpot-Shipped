# Implementation Spec: Save configuration on settings close

**Capability ID**: 194 | **Slug**: auto-save-on-settings-close

## Overview

This capability allows users to configure save configuration on settings close via a configuration variable (CVar). The setting persists across plugin sessions through BakkesMod's CVar system and the data/rocketstats.json configuration file.

## Control Surfaces

- **CVar Name**: `rs_auto_save_on_settings_close`
- **Type**: Boolean / Integer / Float / String
- **Persistence**: BakkesMod CVar system + JSON file
- **Evidence**: RocketStats.cpp:478 (WriteConfig on cl_rocketstats_settings change)
...
**CVar Registration**:
```cpp
cvarManager->registerCvar("rs_auto_save_on_settings_close", "default_value",
...
```json
{
    "rs_auto_save_on_settings_close": "stored_value"
}
```

**Access Pattern**:
```cpp
auto value = cvarManager->getCvar("rs_auto_save_on_settings_close").getValue<Type>();
```
